...
export class CommandHandler {
      ...
      for (const file of commandFiles) {
        ...
        try {
          ...
          if (this.isValidCommand(commandHandler)) {
            ...
            console.log(`[WARNING] The command at ${filePath} is missing or invalid.`)
          }
        } catch (err) {
  ...
  public async registerCommand(): Promise<void> {
    ...
    try {
      console.log('[REGISTER] Started refreshing global application (/) commands.')

      await rest.put(Routes.applicationCommands(this.client.user.id), {
        body: commandData
      })
      console.log('[REGISTER] Successfully reloaded global application (/) commands.')
    } catch (err) {
